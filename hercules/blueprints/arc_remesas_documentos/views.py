"""
Archivo - Remesas Documentos, vistas
"""
...
from hercules.blueprints.arc_remesas_documentos.forms import (
    ArcRemesaDocumentoEditForm,
    ArcRemesaDocumentoArchiveForm,
)
...
def datatable_json():
    ...
    registros = (
        consulta.order_by(ArcDocumento.anio).order_by(ArcDocumento.expediente_numero).offset(start).limit(rows_per_page).all()
    )
        ...
        data.append(
            {
                ...
                "juzgado_origen": {
                    "clave": (
                        resultado.arc_documento.arc_juzgado_origen.clave
                        if resultado.arc_documento.arc_juzgado_origen_id is not None
                        else ""
                    ),
                    "nombre": (
                        resultado.arc_documento.arc_juzgado_origen.descripcion_corta
                        if resultado.arc_documento.arc_juzgado_origen_id is not None
                        else ""
                    ),
                },
...
def edit(arc_remesa_documento_id):
    ...
    # Mostrar el template resultante
    return render_template(
        "arc_remesas_documentos/edit.jinja2",
        remesa=remesa_documento.arc_remesa,
        documento=remesa_documento.arc_documento,
        arc_remesa_documento=remesa_documento,
        form=form,
    ...
    if form.validate_on_submit():
        ...
        if "archivar" in request.form:
            if request.form["archivar"] == "Archivar":
                ...
                remesa_documento.observaciones_archivista = safe_message(
                    form.observaciones_archivista.data, default_output_str=None
                )
                ...
                remesa = ArcRemesa.query.get_or_404(remesa_documento.arc_remesa_id)
                num_anomalias = (
                    ArcRemesaDocumento.query.filter_by(arc_remesa=remesa).filter(ArcRemesaDocumento.anomalia != None).count()
                )
                ...
                documento_bitacora = ArcDocumentoBitacora(
                    arc_documento=documento,
                    usuario=current_user,
                    observaciones=safe_message(
                        f"ANOMALÍA: {remesa_documento.anomalia}. NOTA: {remesa_documento.observaciones_archivista}",
                        default_output_str=None,
                    ),
                    accion="ANOMALIA",
                )
                documento_bitacora.save()
                ...
                if (
                    not "fojas" in request.form
                    or request.form["fojas"] == ""
                    or not "anomalia" in request.form
                    or request.form["anomalia"] == ""
                ):
