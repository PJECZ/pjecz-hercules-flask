...
empunadura = logging.FileHandler("logs/funcionarios.log")
...
def asignar_oficinas(funcionario_id: int, domicilio_id: int):
    ...
    for oficina in oficinas:
        if isinstance(oficina, Oficina):
            ...
            mensaje_error = f"El valor {oficina} no es una instancia válida de Oficina"
        ...
        while True:
            ...
            for persona in personas:
                ...
                if historial_puesto is None:
                    ...
                    en_funciones = False
                if (
                    en_funciones and historial_puesto["fecha_termino"] is None
                ):  # La fecha de termino vacia significa que esta en funciones
                    ...
                    bitacora.info(
                        "A %s (%d) NO tiene un historial de puesto abierto por lo que NO esta en funciones", nombre, persona_id
                    )
                    en_funciones = False
                ...
                if funcionario is None:
                    ...
                    if funcionario_con_mismo_email is not None:
                        bitacora.warning(
                            "Se omite insertar %s (%d) porque el email %s ya esta registrado", nombre, persona_id, email
                        )
                        continue
                    ...
                    if email != funcionario.email:
                        ...
                        if funcionario_con_mismo_email is not None:
                            bitacora.warning(
                                "Se omite actualizar %s (%d) porque el email %s ya esta registrado", nombre, persona_id, email
                            )
                            continue
                        ha_cambiado = True
