...
from hercules.blueprints.arc_solicitudes.forms import ArcSolicitudNewForm, ArcSolicitudAsignationForm, ArcSolicitudFoundForm
        ...
        data.append(
            {
                ...
                "tiempo_recepcion": (
                    "" if resultado.tiempo_recepcion is None else resultado.tiempo_recepcion.strftime("%Y-%m-%d %H:%M:%S")
                ),
                "asignado": {
...
def new(documento_id):
    ...
    if form.validate_on_submit():
        ...
        if documento.ubicacion != "ARCHIVO":
            ...
            ArcSolicitud.query.filter_by(arc_documento=documento).filter_by(esta_archivado=False).filter_by(estatus="A").first()
        ):
...
def cancel(solicitud_id):
    ...
    elif not current_user.can_admin(MODULO) and not {
        ROL_SOLICITANTE,
        ROL_JEFE_REMESA,
        ROL_JEFE_REMESA_ADMINISTRADOR,
    }.intersection(current_user.get_roles()):
...
def no_found(solicitud_id):
    ...
    if solicitud.estado != "ASIGNADO":
        flash(
            "No puede cambiar el estado a NO ENCONTRADO de una solicitud que está en un estado diferente a ASIGNADO.", "warning"
        )
    elif not current_user.can_admin(MODULO) and ROL_ARCHIVISTA not in current_user.get_roles():
...
def print_list():
    ...
    solicitudes = (
        ArcSolicitud.query.join(ArcDocumento)
        .filter(ArcSolicitud.esta_archivado == False)
