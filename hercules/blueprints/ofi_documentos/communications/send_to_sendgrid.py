...
load_dotenv()
HOST = os.getenv("HOST", "https://plataforma-web.justiciadigital.gob.mx")
...
def enviar_a_sendgrid(ofi_documento_id: int) -> tuple[str, str, str]:
    ...
    contenidos.append(f"<li>Remitente: <strong>{ofi_documento.usuario.nombre}</strong></li>")
    ...
    contenidos.append("</ul>")
    contenidos.append("<p>Este mensaje fue elaborado por un programa. <strong>NO RESPONDA ESTE MENSAJE.</strong></p>")
    contenidos.append("<p>Para lograr el CERO PAPEL por favor <strong>NO IMPRIMA ESTE MENSAJE ni el Oficio.</strong></p>")
    contenido_html = "\n".join(contenidos)
    ...
    for destinatario in ofi_destinatarios:
        if destinatario.con_copia == True:
            ...
            mensajes.append(f"- Destinatario con copia: {destinatario.usuario.email}")
