...
from hercules.blueprints.arc_archivos.forms import ArcEstadisticasDateRangeForm
...
def list_active():
    ...
    if ROL_JEFE_REMESA_ADMINISTRADOR in current_user_roles:
        return render_template(
            "arc_archivos/list_jefe_remesa.jinja2",
            filtros_solicitudes=json.dumps(
                {"estatus": "A", "omitir_archivados": True, "omitir_cancelados": True, "sede": current_user.autoridad.sede}
            ),
            filtros_remesas=json.dumps(
                {
                    "estatus": "A",
                    "omitir_archivados": True,
                    "omitir_cancelados": True,
                    "omitir_pendientes": True,
                    "sede": current_user.autoridad.sede,
                }
            ),
            estatus="A",
            titulo="Archivo - Bandeja de Entrada 📥",
    ...
    if ROL_SOLICITANTE in current_user_roles or ROL_RECEPCIONISTA in current_user_roles:
        return render_template(
            ...
            filtros_solicitudes=json.dumps(
                {"estatus": "A", "omitir_archivados": True, "juzgado_id": current_user.autoridad.id}
            ),
...
def list_history():
    ...
    if current_user.can_admin(MODULO):
        return render_template(
            "arc_archivos/list_jefe_remesa.jinja2",
            filtros_solicitudes=json.dumps(
                {"estatus": "A", "mostrar_archivados": True, "omitir_cancelados": False, "orden_acendente": True}
            ),
            filtros_remesas=json.dumps(
                {"estatus": "A", "mostrar_archivados": True, "omitir_cancelados": False, "orden_acendente": True}
            ),
    ...
    if ROL_SOLICITANTE in current_user_roles or ROL_RECEPCIONISTA in current_user_roles:
        return render_template(
            "arc_archivos/list_solicitante.jinja2",
            filtros_solicitudes=json.dumps(
                {"estatus": "A", "mostrar_archivados": True, "juzgado_id": current_user.autoridad.id, "orden_acendente": True}
            ),
            filtros_remesas=json.dumps(
                {"estatus": "A", "mostrar_archivados": True, "juzgado_id": current_user.autoridad.id, "orden_acendente": True}
            ),
    ...
    if ROL_ARCHIVISTA in current_user_roles:
        return render_template(
            "arc_archivos/list_archivista.jinja2",
            filtros_solicitudes=json.dumps(
                {"estatus": "A", "mostrar_archivados": True, "asignado_id": current_user.id, "orden_acendente": True}
            ),
            filtros_remesas=json.dumps(
                {"estatus": "A", "mostrar_archivados": True, "asignado_id": current_user.id, "orden_acendente": True}
            ),
...
def stats_solicitudes():
    ...
    if form.validate():
        ...
        if "totales" in request.form:
            ...
            solicitudes = (
                ArcSolicitud.query.filter(ArcSolicitud.creado >= fecha_desde)
                .filter(ArcSolicitud.creado <= fecha_hasta)
                .filter_by(estatus="A")
                .filter(ArcSolicitud.estado != "CANCELADO")
                .count()
            )
            return render_template(
            ...
            return render_template(
                "arc_archivos/stats_solicitudes_por_archivistas.jinja2",
                fecha_desde=fecha_desde,
                fecha_hasta=fecha_hasta,
...
def stats_remesas():
    ...
    if form.validate():
        ...
        if "totales" in request.form:
            # Cálculo de solicitudes totales
            remesas = (
                ArcRemesa.query.filter(ArcRemesa.creado >= fecha_desde)
                .filter(ArcRemesa.creado <= fecha_hasta)
                .filter_by(estatus="A")
                .filter(ArcRemesa.estado != "CANCELADO")
                .filter(ArcRemesa.estado != "PENDIENTE")
                .count()
            )
            return render_template(
...
def datatable_json_solicitudes_por_distrito():
    ...
    consulta = (
        consulta.filter(Distrito.es_distrito == True)
        .filter(Distrito.es_distrito_judicial == True)
        .filter(Distrito.es_jurisdiccional == True)
    )
    ...
    consulta = (
        consulta.filter(Autoridad.estatus == "A")
        .filter(ArcSolicitud.estatus == "A")
...
def datatable_json_remesas_por_instancias():
    ...
    if "fecha_hasta" in request.form:
        consulta = consulta.filter(ArcRemesa.creado <= request.form["fecha_hasta"])
    consulta = (
        consulta.filter(ArcRemesa.estatus == "A")
        .filter(ArcRemesa.estatus == "A")
        .filter(ArcRemesa.estado != "CANCELADO")
        .filter(ArcRemesa.estado != "PENDIENTE")
    )
...
@arc_archivos.route("/arc_archivos/datatable_json_solicitudes_por_archivistas", methods=["GET", "POST"])
def datatable_json_solicitudes_por_archivistas():
    """DataTable JSON para listado de solicitudes y remesas por archivistas"""
    # Tomar parámetros de Datatables
    draw, start, rows_per_page = get_datatable_parameters()
    # SQLAlchemy database session
